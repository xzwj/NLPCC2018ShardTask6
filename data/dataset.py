...
class StackData(data.Dataset):
    def __init__(self,data_root,labels_file,val):
        ...
        self.train_data=t.zeros(self.all_num-self.val_num,25551*self.model_num)
        self.val_data=t.zeros(self.val_num,25551*self.model_num)
        for i in range(self.model_num):
            tmpdata=t.load(self.data_files_path[i]).float()
            self.train_data[:,i*25551:i*25551+25551]=tmpdata[:self.all_num-self.val_num]
            self.val_data[:,i*25551:i*25551+25551]=tmpdata[-1*self.val_num:]
    ...
    def __getitem__(self,index):
        ...
        data = data.float()
        label_tensor = t.zeros(25551).scatter_(0,t.LongTensor(label),1).long()
